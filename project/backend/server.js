...
const imageSize = require('image-size'); // To validate image dimensions
const fileType = require('file-type'); // To validate file type
...
const upload = multer({
  ...
  limits: {
    ...
    files: 1, // One file at a time to avoid cumulative issues
  },
...
app.post('/api/upload-image', upload.single('file'), async (req, res) => {
  try {
    ...
    // Validate image dimensions (ensure it's a valid image)
    ...
    // Use a stream to upload the file to S3, preserving quality
...
// Other routes (slides, memorias, docentes, etc.) remain unchanged
